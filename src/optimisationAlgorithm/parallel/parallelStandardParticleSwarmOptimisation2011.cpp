    ...
    while(!isFinished() && !isTerminated()) {
      for(unsigned int k = 0; k < communicationSteps_; ++k) {
        ...
        for(std::size_t n = 0; n < localPopulationSize_; ++n) {
          ...
          arma::Col<double> particle = localParticles_.col(nn);
          ...
          arma::Col<double> velocityCandidate = acceleration_ * localVelocities_.col(nn) + arma::normalise(arma::randn<arma::Col<double>>(optimisationProblem_->getNumberOfDimensions())) * std::uniform_real_distribution<double>(0, 1)(Random::Rng) * arma::norm(attractionCenter) + attractionCenter;
          ...
          localVelocities_.col(nn) = velocityCandidate;
          localParticles_.col(nn) = solutionCandidate;
