...
namespace mant {
  arma::Mat<double> hammersleySet(
    ...
    // The last element is equal to n/*numberOfColumms* (n = the dimension's index).
    for (arma::uword n = 0; n < seed.n_elem; ++n) {
      sequence.submat(n, 0, n, numberOfColumms - 2) = vanDerCorputSequence(base(n), seed(n), numberOfColumms - 1).t();
      sequence.submat(n, numberOfColumms - 1, n, numberOfColumms - 1) = static_cast<double>(n + 1) / static_cast<double>(numberOfColumms);
    }
  ...
  arma::Mat<double> haltonSequence(
    ...
    for (arma::uword n = 0; n < seed.n_elem; ++n) {
      sequence.row(n) = vanDerCorputSequence(base(n), seed(n), numberOfColumms).t();
    }
