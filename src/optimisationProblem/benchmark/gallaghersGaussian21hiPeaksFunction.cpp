...
namespace hop {
  GallaghersGaussian21hiPeaksFunction::GallaghersGaussian21hiPeaksFunction(const unsigned int &numberOfDimensions) : BenchmarkProblem(numberOfDimensions), _rotationR(getRandomRotation()), _delta(numberOfDimensions, 21), _localOptima(numberOfDimensions, 21), _weight(21) {
    _delta.col(0) = getScaling(sqrt(1000.0)) / pow(1000000.0, 0.25);

    uniform_int_distribution<int> uniformIntDistribution(0, 19);
    for(size_t j = 1; j < _delta.n_cols; j++) {
      _delta.col(j) = getScaling(sqrt(1000.0)) / pow(pow(1000.0, 2.0 * static_cast<double>(uniformIntDistribution(Random::RNG)) / 19.0), 0.25);
    }
