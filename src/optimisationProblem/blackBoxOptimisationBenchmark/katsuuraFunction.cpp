  ...
  namespace bbob {
    KatsuuraFunction::KatsuuraFunction(
      ...
      if (numberOfDimensions_ < 2) {
        throw std::domain_error("KatsuuraFunction: The number of dimensions must be greater than 1.");
      } else if (!isRepresentableAsFloatingPoint(numberOfDimensions_)) {
        throw std::overflow_error("KatsuuraFunction: The number of elements must be representable as a floating point.");
      }
