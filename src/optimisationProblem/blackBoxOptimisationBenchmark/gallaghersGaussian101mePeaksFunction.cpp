  ...
  namespace bbob {
    GallaghersGaussian101mePeaksFunction::GallaghersGaussian101mePeaksFunction(
        ...
        : BlackBoxOptimisationBenchmark(numberOfDimensions),
          ...
          localParameterTranslations_(synchronise(arma::mat(arma::randu<arma::mat>(numberOfDimensions_, 101) * 10.0 - 5.0))) {
      if (numberOfDimensions_ < 2) {
        throw std::domain_error("GallaghersGaussian101mePeaksFunction: The number of dimensions must be greater than 1.");
      } else if (!isRepresentableAsFloatingPoint(numberOfDimensions_)) {
        throw std::overflow_error("GallaghersGaussian101mePeaksFunction: The number of elements must be representable as a floating point.");
      }
      ...
      // Only the condition numbers are randomly drawn and therefore need to be synchronise.
