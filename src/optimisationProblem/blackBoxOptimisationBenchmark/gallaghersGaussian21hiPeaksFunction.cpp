  ...
  namespace bbob {
    GallaghersGaussian21hiPeaksFunction::GallaghersGaussian21hiPeaksFunction(
        ...
        : BlackBoxOptimisationBenchmark(numberOfDimensions),
          ...
          localParameterTranslations_(synchronise(arma::mat(arma::randu<arma::mat>(numberOfDimensions_, 21) * 9.8 - 4.9))) {
      if (numberOfDimensions_ < 2) {
        throw std::domain_error("GallaghersGaussian21hiPeaksFunction: The number of dimensions must be greater than 1.");
      } else if (!isRepresentableAsFloatingPoint(numberOfDimensions_)) {
        throw std::overflow_error("GallaghersGaussian21hiPeaksFunction: The number of elements must be representable as a floating point.");
      }
