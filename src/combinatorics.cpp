...
// Since factorials grow within a few steps larger than the largest number representable by `arma::uword`, a good way to *calculate* the factorial is to simply lookup its value.
    ...
    if (n > 20) {
      throw std::overflow_error("factorial: Factorials larger as 20! are not supported in 64-bit mode.");
    }
...
#else
    if (n > 12) {
      throw std::overflow_error("factorial: Factorials larger as 12! are not supported in 32-bit mode.");
    }
  ...
  arma::uword secondStirlingNumber(
    ...
    if (!isRepresentableAsFloatingPoint(numberOfElements - 1)) {
      throw std::domain_error("secondStirlingNumber: The number of elements must be representable as a floating point.");
    } else if (!isRepresentableAsFloatingPoint(numberOfParts - 1)) {
      throw std::domain_error("secondStirlingNumber: The number of parts must be representable as a floating point.");
    }
  ...
  std::vector<arma::uvec> combinations(
    ...
    if (numberOfElements == 0) {
      throw std::domain_error("combinations: The combination size must be greater than 0.");
    } else if (cycleSize > numberOfElements) {
      throw std::logic_error("combinations: The number of elements must be greater than or equal to the combination size.");
    }
    ...
    if (numberOfElements == cycleSize) {
      ...
      if (cycleSize > std::numeric_limits<decltype(bitmask)::difference_type>::max()) {
        throw std::overflow_error("combinations: The combination size must be less than or equal to the largest representable iterator difference.");
      }
  ...
  std::vector<arma::uvec> multicombinations(
      ...
      const arma::uword cycleSize) {
    if (numberOfElements == 0) {
      throw std::domain_error("multicombinations: The number of elements must be greater than 0.");
    }
  ...
  std::vector<std::pair<arma::uvec, arma::uvec>> twoSetsPartitions(
    ...
    if (numberOfElements < 2) {
      throw std::domain_error("twoSetsPartitions: The number of elements must be greater than 1.");
      ;
    ...
    if (numberOfElements / 2 > std::numeric_limits<std::vector<arma::uword>::difference_type>::max()) {
      throw std::overflow_error("twoSetsPartitions: The halved number of elements must be less than or equal to the largest representable iterator difference.");
    }
    ...
    for (arma::uword n = 1; n <= numberOfElements / 2; ++n) {
      ...
      do {
        /* Avoids adding duplicates when we split the elements in equally sized parts, skipping the second half of permutations in this case.
         * If `numberOfElements` was set to 4, we would get:
         *
         * n = 2
         * (1, 1, 0, 0) (Bit mask)
