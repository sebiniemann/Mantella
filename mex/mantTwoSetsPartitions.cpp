...
void mexFunction(
  ...
  try {
    ...
    mxArray* twoSetsPartitions = mxCreateCellMatrix(numberOfTwoSetPartitions, 2);
    if (twoSetsPartitions == nullptr) {
      ...
      for (const auto& twoSetsPartition : mant::twoSetsPartitions(numberOfElements)) {
        mxSetCell(twoSetsPartitions, n, getMxArray(arma::conv_to<arma::vec>::from(twoSetsPartition.first)));
        mxSetCell(twoSetsPartitions, n + numberOfTwoSetPartitions, getMxArray(arma::conv_to<arma::vec>::from(twoSetsPartition.second)));
        ++n;
      }
