...
void mexFunction(
  ...
  try {
    bbob::SchaffersF7Function optimisationProblem(getIntegerScalar(prhs[0]));

    mxArray* serialisedOptimisationProblem = getMxArray(static_cast<mant::BlackBoxOptimisationBenchmark>(optimisationProblem));
    mxSetField(serialisedOptimisationProblem, 0, "functionHandle", mxCreateString("BBOB Schaffers F7 Function (f17)"));
