...
void mexFunction(
  ...
  try {
    ...
    arma::uword n = 0;
    for (const auto& circleSphereIntersection : mant::circleSphereIntersections(getRealVector(prhs[0]), getRealScalar(prhs[1]), getRealVector(prhs[2]), getRealVector(prhs[3]), getRealScalar(prhs[4]))) {
