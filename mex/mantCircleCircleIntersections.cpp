...
void mexFunction(
  ...
  try {
    ...
    arma::uword n = 0;
    for (const auto& circleCircleIntersection : mant::circleCircleIntersections(getRealVector(prhs[0]), getRealScalar(prhs[1]), getRealVector(prhs[2]), getRealScalar(prhs[3]))) {
