...
int main (const int argc, const char* argv[]) {
  ...
  list.load("parameterisations.mat");
      ...
      mant::ParallelStandardParticleSwarmOptimisation2011 optimisationAlgorithm(optimisationProblem, (std::stoi(list.at(parameterisationRow, 2))));
      ...
      for(auto communicationStep : communicationSteps) {
          ...
          for(std::size_t n = 0; n < iterations; ++n) {
            ...
            totalNumberOfIterations.at(n) = optimisationAlgorithm.getNumberOfIterations();

            if (optimisationAlgorithm.isFinished()){
             temp.at(n) = optimisationAlgorithm.getNumberOfIterations();
           } else {
              temp.at(n) = 0;
            }
          ...
          totalNumberOfIterations.save("./total/totalParallelNumberOfIterations_" + list.at(parameterisationRow,0) + "_dim" + list.at(parameterisationRow, 1) + "_pop" + list.at(parameterisationRow, 2) + "_step" + std::to_string(communicationStep) + ".mat", arma::raw_ascii);
          numberOfItreations.save("./parallelNumberOfIterations_" + list.at(parameterisationRow,0) + "_dim" + list.at(parameterisationRow, 1) + "_pop" + list.at(parameterisationRow, 2) + "_step" + std::to_string(communicationStep) + ".mat", arma::raw_ascii);
