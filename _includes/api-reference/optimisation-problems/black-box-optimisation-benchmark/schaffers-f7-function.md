...
Z &:= T_\text{scaled}^\sqrt{10} \cdot Q \cdot T_\text{asymmetric}^{0.5}\left( R \cdot \left( X - X^\text{opt} \right) \right)\\
S_i &:= \left(Z_i^2 + Z_{i+1}^2 \right)^{0.25}
\end{align}$$

**SchaffersF7Function( <small>unsigned int</small> N )**
...
- Returns a filesystem friendly name of the problem, i.e. *schaffers-f7-function*.