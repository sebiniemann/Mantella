...
echo "${MAGENTA_TEXT_COLOR}Minifying jpegs${RESET_TEXT_COLOR}";
FILES=$(find assets/images/ -type f -name \*.jpg);
...
echo "${MAGENTA_TEXT_COLOR}Minifying pngs${RESET_TEXT_COLOR}";
FILES=$(find assets/images/ -type f -name \*.png);
NUMBER_OF_FILES=$(echo "${FILES}" | wc -l);
COUNTER=1;
...
while read FILE; do
  ...
  pngquant --force 256 --ext .png ${FILE}
