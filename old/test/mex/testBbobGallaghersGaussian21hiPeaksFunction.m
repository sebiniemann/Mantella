...
void mexFunction(
  ...
  try {
    bbob::GallaghersGaussian21hiPeaksFunction optimisationProblem(getIntegerScalar(prhs[0]));

    mxArray* serialisedOptimisationProblem = getMxArray(static_cast<mant::BlackBoxOptimisationBenchmark>(optimisationProblem));
    mxSetField(serialisedOptimisationProblem, 0, "functionHandle", mxCreateString("BBOB Gallagher's Gaussian 21-hi Peaks Function (f22)"));
