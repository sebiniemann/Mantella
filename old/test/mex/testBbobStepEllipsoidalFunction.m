...
void mexFunction(
  ...
  try {
    bbob::StepEllipsoidalFunction optimisationProblem(getIntegerScalar(prhs[0]));

    mxArray* serialisedOptimisationProblem = getMxArray(static_cast<mant::BlackBoxOptimisationBenchmark>(optimisationProblem));
    mxSetField(serialisedOptimisationProblem, 0, "functionHandle", mxCreateString("BBOB Step Ellipsoidal Function (f7)"));
