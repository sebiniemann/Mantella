...
SCENARIO("operator==", "[testHelper][operator==]") {
  GIVEN("Two maps") {
...
SCENARIO("hasSameElements", "[testHelper][hasSameElements]") {
...
SCENARIO("hasSameDistribution", "[testHelper][hasSameDistribution]") {
...
SCENARIO("isUniformDistributed", "[testHelper][isUniformDistributed]") {
  GIVEN("A vector") {
...
SCENARIO("isNormalDistributed", "[testHelper][isNormalDistributed]") {
  GIVEN("A vector") {
    WHEN("The vector is empty") {
      THEN("Return false") {
...
SCENARIO("isCauchyDistributed", "[testHelper][isCauchyDistributed]") {
  GIVEN("A vector") {
    WHEN("The vector is empty") {
      THEN("Return false") {
    ...
    WHEN("The vector is Cauchy distributed") {
      THEN("Return true") {
        arma::vec::fixed<10000> cauchyDistribution;
        for (arma::uword n = 0; n < cauchyDistribution.n_elem; ++n) {
          cauchyDistribution(n) = std::cauchy_distribution<double>(0.0, 10.0)(mant::Rng::getGenerator());
        }
        CHECK(isCauchyDistributed(cauchyDistribution, 10.0) == true);
      }
    }
  }
}
