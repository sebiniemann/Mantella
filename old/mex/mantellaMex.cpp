...
arma::vec getRealColumnVector(
    const mxArray* data) {
  if (data == nullptr) {
    mexErrMsgTxt("getRealColumnVector: The data must exist, i.e. not point to a null pointer.");
  } else if (mxIsSparse(data)) {
    mexErrMsgTxt("getRealColumnVector: The data must point to a dense (not sparse) matrix.");
  } else if (!mxIsEmpty(data) && !(mxGetNumberOfDimensions(data) == 2 && (mxGetM(data) == 1 || mxGetN(data) == 1))) {
    mexErrMsgTxt("getRealColumnVector: The data must point to a vector (if filled).");
  } else if (!mxIsDouble(data)) {
    mexErrMsgTxt("getRealColumnVector: The data point to double precision floating point values.");
  }
...
std::shared_ptr<mant::OptimisationProblem> getOptimisationProblem(
  ...
  optimisationProblem->setLowerBounds(getRealColumnVector(mxGetField(serialisedOptimisationProblem, 0, "lowerBounds")));
  optimisationProblem->setUpperBounds(getRealColumnVector(mxGetField(serialisedOptimisationProblem, 0, "upperBounds")));
  ...
  optimisationProblem->setParameterScaling(getRealColumnVector(mxGetField(serialisedOptimisationProblem, 0, "parameterScaling")));
  optimisationProblem->setParameterTranslation(getRealColumnVector(mxGetField(serialisedOptimisationProblem, 0, "parameterTranslation")));
...
mxArray* getMxArray(
  ...
  mxArray* serialisedOptimisationProblem = mxCreateStructMatrix(1, 1, 10, fieldNames);
