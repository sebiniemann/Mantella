          ...
          {{[this](
              ...
              for (arma::uword n = 0; n < endEffectorTrajectory_.n_cols; ++n) {
                ...
                for (arma::uword k = 0; k < baseJointsPosition.n_cols; ++k) {
                  const arma::mat& intersections = circleCircleIntersections(baseJointsPosition.col(k), linkLengths_(0, k), endEffectorJointsPosition.col(k), linkLengths_(1, k));

                  if (intersections.n_cols > 1) {
                    middleJointsPosition.col(k) = intersections.col(0);
                  } else {
