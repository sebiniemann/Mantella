...
namespace mant {
  ...
  arma::mat::fixed<3, 3> rotationMatrix3d(
    ...
    if (std::abs(std::fmod(pitchAngle, arma::datum::pi / 2.0)) < ::mant::machinePrecision) {
      // Uses quaternions
      const double sinHalvedRollAngle = std::sin(rollAngle / 2.0);
      const double cosHalvedRollAngle = std::cos(rollAngle / 2.0);
      const double sinHalvedPitchAngle = std::sin(pitchAngle / 2.0);
      const double cosHalvedPitchAngle = std::cos(pitchAngle / 2.0);
      const double sinHalvedYawAngle = std::sin(yawAngle / 2.0);
      const double cosHalvedYawAngle = std::cos(yawAngle / 2.0);
      const arma::vec& quaternion = {
          cosHalvedRollAngle * cosHalvedPitchAngle * cosHalvedYawAngle + sinHalvedRollAngle * sinHalvedPitchAngle * sinHalvedYawAngle,
          sinHalvedRollAngle * cosHalvedPitchAngle * cosHalvedYawAngle - cosHalvedRollAngle * sinHalvedPitchAngle * sinHalvedYawAngle,
          cosHalvedRollAngle * sinHalvedPitchAngle * cosHalvedYawAngle + sinHalvedRollAngle * cosHalvedPitchAngle * sinHalvedYawAngle,
          cosHalvedRollAngle * cosHalvedPitchAngle * sinHalvedYawAngle - sinHalvedRollAngle * sinHalvedPitchAngle * cosHalvedYawAngle};
      ...
      const double sinRollAngle = std::sin(rollAngle);
      const double cosRollAngle = std::cos(rollAngle);
      const double sinPitchAngle = std::sin(pitchAngle);
      const double cosPitchAngle = std::cos(pitchAngle);
      const double sinYawAngle = std::sin(yawAngle);
      const double cosYawAngle = std::cos(yawAngle);
      return arma::mat(
          {{cosYawAngle * cosPitchAngle,
            cosYawAngle * sinPitchAngle * sinRollAngle - sinYawAngle * cosRollAngle,
            cosYawAngle * sinPitchAngle * cosRollAngle + sinYawAngle * sinRollAngle},
           {sinYawAngle * cosPitchAngle,
            sinYawAngle * sinPitchAngle * sinRollAngle + cosYawAngle * cosRollAngle,
            sinYawAngle * sinPitchAngle * cosRollAngle - cosYawAngle * sinRollAngle},
           {-sinPitchAngle,
            cosPitchAngle * sinRollAngle,
            cosPitchAngle * cosRollAngle}});
    }
  }

  arma::mat circleCircleIntersections(
  ...
  arma::mat circleSphereIntersections(
    ...
    if (std::abs(circleRadius - std::abs(x)) < ::mant::machinePrecision) {
      ...
      return arma::join_rows(circleCentre + x * xUnitVector + y * yUnitVector, circleCentre + x * xUnitVector - y * yUnitVector);
    }
  }
}
