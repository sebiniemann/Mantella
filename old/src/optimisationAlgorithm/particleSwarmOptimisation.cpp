    ...
    setNextParametersFunctions(
        {{[this](
            ...
            for (arma::uword n = 0; n < populationSize_; ++n) {
              ...
              const arma::vec& attractionCenter = (getMaximalLocalAttraction() * (localBestSolutions_.col(n) - particle) + getMaximalGlobalAttraction() * (getBestFoundParameter() - particle)) / 3.0;
