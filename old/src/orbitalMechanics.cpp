...
namespace mant {
  ...
  double timeOfFlight(
    ...
    double timeOfFlight = (std::pow(y / secondStumpffValue, 1.5) * thirdStumpffValue + A * std::sqrt(y)) / std::sqrt(::mant::heliocentricGravitationalConstant);
  ...
  double sphereOfInfluenceRadius(
    ...
    return std::pow(mass / ::mant::solarMass, 0.4) * semiMajorAxis;
  }
  ...
  std::pair<arma::vec::fixed<3>, arma::vec::fixed<3>> positionAndVelocityOnOrbit(
    ...
    const double meanAnomaly = keplerianElements(5) + (modifiedJulianDate - keplerianElements(6)) * std::sqrt(::mant::heliocentricGravitationalConstant / std::pow(semiMajorAxis, 3.0));
    ...
    const double n = std::sqrt(::mant::heliocentricGravitationalConstant / std::pow(semiMajorAxis, 3.0));
  ...
  std::pair<arma::vec::fixed<3>, arma::vec::fixed<3>> lambert(
    ...
    const double secondLagrangeCoefficient = A * std::sqrt(y / ::mant::heliocentricGravitationalConstant);
    const double firstLagrangeCoefficientDeriavte = std::sqrt(::mant::heliocentricGravitationalConstant) / (departureDistanceToSun * arrivalDistanceToSun) * std::sqrt(y / secondStumpffValue) * (universalVariable * thirdStumpffValue - 1.0);
    const double secondLagrangeCoefficientDeriavte = 1.0 - y / arrivalDistanceToSun;

    const arma::vec& departureVelocity = 1.0 / secondLagrangeCoefficient * (arrivalPosition - firstLagrangeCoefficient * departurePosition);
