...
.. cpp:function:: additive_separability(problem, evaluations, acceptable_deviation)
  ...
  Additive separability means, that a function `f` can be split into the summation over sub-functions (`g`, `h`, ...), along with its dimensions.

  Within the context of optimisation, this means that instead of solving for example a `5`-dimensional problem, it could be possible to split it into `2`- and `3`-dimensional problem. This would lead into a significant speed-up for the optimiser, as each invokation can happen in parallel and needs to handle a way samller search space.

  Mathematically speaking, a function `f` is additive separable into two less dimensional functions `g`, `h` iff:

  .. math::

    f(x, y) - g(x) - h(y) = 0, for all x, y

  As it is practical impossible to get two separations `g`, `h` of `f`, when we can only call `.objective_function` and have no analytical, symbolic representation, we use a direct consequence from the above equation instead:

  .. math::

    f(a, c) + f(b, d) - f(a, d) - f(b, c) = 0, for all a, b, c, d

  The key challange is therefore to know whether or not a function is separable and what is the separation with the smallest maximal number of dimensions.

  .. container:: topic

    From analysis to the practical usage

    Simply calling :cpp:any:`additive_separability` to analyse a function will give us information about its separability, but will not immedialty split the function into several parts, as we still do not know the sub-functions `g`, `h`, ... since we have no analytical, symbolic representation of the problem.

    To make use of its separability, we have to incorperate this knowledge into the :cpp:any:`optimiser` instead. For this purpose, we added the parameter `.active_dimensions` into the optimiser, denoting which dimensions are to be optimised and which are to be left untouched.

    By invoking the optimisation process for each separation, while setting `.active_dimensions` the separation's dependent dimensions, we can solve the whole problem while reducing the number of dimensions for each invokation.
  ...
  .. list-table:: Function functions
    ...
    * - evaluations
        ...
        ``std::size_t``
      - The number of evaluations per separation to try. If an evaluation is found that exceeds ``acceptable_deviation``, the separation candidate is discarded and the next one is tried.
...
std::array<std::size_t, N> additive_separability(
  ...
  // Note: *n* denotes the number of elements in the first part (and is therefore limited by floor (*N* / 2)).
  for (std::size_t n = 1; n <= N / 2; ++n) {
    /* Each partition is represented by a bitmask, whereby *true* marks elements in the first part and *false* marks elements in the second part.
    ...
    do {
      /* Avoids adding duplicates when we split the elements in equally sized parts, skipping the second half of permutations.
       *
      ...
      for (std::size_t k = 0; k < evaluations; ++k) {
         ...
         * A function *f* is additive separable into two less dimensional functions *g*, *h* iff:
         *
         * f(x, y) - g(x) - h(y) = 0, for all x, y
         *
         * As it is practical impossible to get two separations *g*, *h* of *f*, when we only got a caller to *f* and no analytic/symbolic form, we use a direct consequence from the above equation instead, that must also hold true for additive separable functions and uses only *f*:
         *
         * f(a, c) + f(b, d) - f(a, d) - f(b, c) = 0, for all a, b, c, d
         *
         */
        ...
        for (std::size_t l = 0; l < N; ++l) {
          // Maps [0, 1] linear to [*problem.lower_bounds*, *problem.upper_bounds*], addressing the actual bounds per dimension.
          parameter_ac.at(l) =
         ...
         * For the given example, the resulting partition would therefore be (0, 1, 1, 2), separating the function into three parts.
         *
         * In order to avoid storing all bit masks and doing it incrementally instead, we compare the current bit mask with the current *partition* instead.
         *
         *   (1, 1, 1, 0) (Current bit mask)
         *   (0, 1, 1, 2) (Current *partition*)
         *
         * Analogous to the statement above, two dimensions remain in the same part, if the sequence along their column axis is equal.
         */
        ...
        for (std::size_t l = 0; l < N; ++l) {
          if (is_remaining_dimension.at(l)) {
            ...
            // If the sequence matches, both get the same part number (*l*) and the dimension is marked as non-remaining.
