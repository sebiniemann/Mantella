...
std::array<T, number_of_elements> random_neighbour(
  ...
  std::generate(neighbour.begin(), neighbour.end(), std::bind(std::normal_distribution<T>(), random_number_generator()));
  const T scaled_length = (minimal_distance + std::uniform_real_distribution<T>(0, 1)(random_number_generator()) * (maximal_distance - minimal_distance)) / std::sqrt(std::accumulate(neighbour.begin(), neighbour.end(), 0.0, [](const T length, const T element) {return length + std::pow(element, 2.0);}));
  
  for (std::size_t element_index = 0; element_index < number_of_elements; ++element_index) {
    neighbour[element_index] = parameter[element_index] + neighbour[element_index] * scaled_length;
  }
...
TEST_CASE("random_neighbour", "[randomisation][random_neighbour]") {
  ...
  for (std::size_t n = 0; n < number_of_repetitions; ++n) {
    ...
    lengths[n] = (std::sqrt(std::accumulate(neighbour.begin(), neighbour.end(), 0.0, [](const double length, const double element) {return length + std::pow(element, 2.0);})) - minimal_distance) / (maximal_distance - minimal_distance);
  ...
  for (const auto length : lengths) {
    ...
    histogram[std::min<std::size_t>(length * number_of_bins, number_of_bins - 1)] += 1.0 / static_cast<double>(lengths.size());
  }
  CHECK(std::accumulate(histogram.cbegin(), histogram.cend(), 0.0, [](const double entropy, const double element) {return entropy + -std::log(element) * element;}) > std::log(histogram.size()) * 0.9);
  
  histogram.fill(0.0);
  for (const auto angle : angles) {
    // Ensures that the index will never be out-of-range (esp. in case *angle* is 1).
    histogram[std::min<std::size_t>(angle * number_of_bins, number_of_bins - 1)] += 1.0 / static_cast<double>(lengths.size());
  }
  CHECK(std::accumulate(histogram.cbegin(), histogram.cend(), 0.0, [](const double entropy, const double element) {return entropy + -std::log(element) * element;}) > std::log(histogram.size()) * 0.9);
}
#endif