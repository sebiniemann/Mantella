namespace mant {
  namespace robotic {
    ...
    inline arma::Cube<double>::fixed<3, 3, 3> ParallelKinematicMachine3PRUS::getModel(
      ...
      checkCompatibleDimension("The number of redundant actuations", redundantJointActuations.n_elem, "the number of redundant joints", redundantJointIndicies_.n_elem);
    ...
    inline double ParallelKinematicMachine3PRUS::getEndEffectorPoseAccuracy(
      ...
      for (std::size_t n = 0; n < redundantJointIndicies_.n_elem; ++n) {
        const unsigned int& redundantJointIndex = redundantJointIndicies_(n);
        inverseKinematic(n, 3 + n) = arma::dot(baseToEndEffectorJoints.col(redundantJointIndex), redundantJointAngles_.col(redundantJointIndex));
      }
