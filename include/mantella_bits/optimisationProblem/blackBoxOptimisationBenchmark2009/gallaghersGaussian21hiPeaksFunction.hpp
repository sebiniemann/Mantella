  ...
  namespace bbob2009 {
    ...
    inline double GallaghersGaussian21hiPeaksFunction::getObjectiveValueImplementation(
      ...
      for (unsigned int k = 0; k < 21; ++k) {
        ...
        maximalValue = std::max(maximalValue, weight_(k) * std::exp(-1.0 / (2.0 * static_cast<double>(numberOfDimensions_)) * arma::dot(parameterTransposition, rotationR_.t() * arma::diagmat(deltaC21_.col(k)) * rotationR_ * parameterTransposition)));
      }
