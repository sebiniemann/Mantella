namespace mant {
  namespace bbob {
    ...
    template <typename T>
    double AttractiveSectorFunction<T>::getObjectiveValueImplementation(
      ...
      return std::pow(this->getOscillatedValue(std::pow(static_cast<double>(arma::norm(z)), 2.0)), 0.9);
    }
