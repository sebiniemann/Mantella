...
namespace hop {
  // Helper class providing geometric features (rotations, intersections, ..).
  class Geometry {
    public:
      // Generates a 2-dimensional right-handside rotation matrix.
      static arma::Mat<double>::fixed<2, 2> get2DRotationMatrix(
          // Rotation around the x-axis.
          const double& angle);

      // Generates a 3-dimensional right-handside rotation matrix.
      static arma::Mat<double>::fixed<3, 3> get3DRotationMatrix(
          // Rotation around the x-axis.
      ...
      static arma::Col<double>::fixed<2> getCircleCircleIntersection(
      ...
      // Note: Based on the usage of this helper function, we only considers cases with exactly two
      // intersactions a valid.
      static arma::Col<double>::fixed<3> getCircleSphereIntersection(
