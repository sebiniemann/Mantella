...
public class WorkSpaceMesh extends Mesh {
  ...
  private Mat armMatrix = null;
  private float distanceArmEndToEndEffector;

  public WorkSpaceMesh(Mat armMatrix, float distanceArmEndToEndEffector) {
    this.armMatrix = armMatrix;
    this.distanceArmEndToEndEffector = distanceArmEndToEndEffector;

    //We need all the circles first, naturally
    ArrayList<Circle> circles = getCircles(this.armMatrix);
    ...
    vertices.add(new Vector3f(sortedIntersects.get(0).x, sortedIntersects.get(0).y, 0));
    ...
    for (int i = 0; i < sortedIntersects.size(); i++) {
      ...
      while (isSmaller(angle, stepping, endingAngle)) {
        ...
        float x = currCircle.x + currCircle.radius * FastMath.cos(currAngle);
        float y = currCircle.y + currCircle.radius * FastMath.sin(currAngle);
      ...
      if (i == sortedIntersects.size() - 1) {
        endPoint = new Vector3f(sortedIntersects.get(0).x, sortedIntersects.get(0).y, 0);
      } else {
        endPoint = new Vector3f(sortedIntersects.get(i + 1).x, sortedIntersects.get(i + 1).y, 0);
      }
  ...
  private float calcArmLength(int index) {
    float length = 0;
    for (int i = 0; i < armMatrix.n_cols / 2; i += 2) {
      Vector2f curr = new Vector2f((float) armMatrix.at(index, i), (float) armMatrix.at(index, i + 1));
      Vector2f next = new Vector2f((float) armMatrix.at(index, i + 2), (float) armMatrix.at(index, i + 3));
  ...
  private Intersect[] getIntersectsFor(Circle first, Circle second) {
    float distance = FastMath.sqrt(FastMath.sqr(first.x - second.x) + FastMath.sqr(first.y - second.y));
    float a = (FastMath.sqr(first.radius) - FastMath.sqr(second.radius) + FastMath.sqr(distance)) / (2 * distance);
    float b = distance - a;
    float h = FastMath.sqrt(FastMath.sqr(first.radius) - FastMath.sqr(a));
    Vector2f centerPoint = new Vector2f(first.x + (a * (second.x - first.x) / distance), first.y + (a * (second.y - first.y) / distance));
    //float upperX = middlePoint.x = 
    Intersect upper = new Intersect(centerPoint.x + (h * (second.y - first.y) / distance), centerPoint.y - (h * (second.x - first.x) / distance), first, second);
    Intersect lower = new Intersect(centerPoint.x - (h * (second.y - first.y) / distance), centerPoint.y + (h * (second.x - first.x) / distance), first, second);
  ...
  private ArrayList<Circle> getCircles(Mat armMatrix) {
    ...
    for (int i = 0; i < armMatrix.n_rows; i++) {
      float length = calcArmLength(i) + distanceArmEndToEndEffector;
  ...
  private ArrayList<Intersect> getIntersects(ArrayList<Circle> circles) {
    ...
    if (circles.size() == 1) {
      ...
      for (Intersect[] is : tempIntersects) {
        float dist1 = centre.distance(new Vector2f(is[0].x, is[0].y));
        float dist2 = centre.distance(new Vector2f(is[1].x, is[1].y));
        if (dist1 < dist2) {
          intersects.add(is[0]);
        } else {
          intersects.add(is[1]);
  ...
  private ArrayList<Intersect> sortIntersects(ArrayList<Intersect> intersects, Vector2f centrePoint) {
    ...
    for (Intersect is : intersects) {
      float angle = centrePoint.angleBetween(new Vector2f(is.x, is.y));
      angles.add(angle);
  ...
  private Circle getCircleFor(Intersect first, Intersect second) {
    if (first.first.equals(second.first) || first.first.equals(second.second)) {
      return first.first;
    } else if (first.second.equals(second.first) || first.second.equals(second.second)) {
      return first.second;
    } else {
      return null;
    }
  ...
  private float getAngleFor(Circle circle, Intersect intersect) {
    ...
    return FastMath.atan2(intersect.y - circle.y, intersect.x - circle.x);
  }
