...
TEST_CASE("bbob2012::RastriginFunction", "") {
  for (const auto& numberOfDimensions : {2, 40}) {
    mant::bbob2012::RastriginFunction rastriginFunction(numberOfDimensions);
  ...
  SECTION("Returns the specified class name.") {
    CHECK(mant::bbob2012::RastriginFunction(5).toString() == "rastrigin-function");
  }
}
