...
TEST_CASE("AttractiveSectorFunction", "") {
  for (const auto& numberOfDimensions : {2, 40}) {
    hop::bbob2015::AttractiveSectorFunction attractiveSectorFunction(numberOfDimensions);
    ...
    expected.load(testDirectory.string() + "/data/optimisationProblem/blackBoxOptimisationBenchmark2015/expectedAttractiveSectorFunction,dim" + std::to_string(numberOfDimensions) +".mat");
