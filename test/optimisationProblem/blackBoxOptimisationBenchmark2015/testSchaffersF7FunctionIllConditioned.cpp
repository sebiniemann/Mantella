...
TEST_CASE("bbob2015::SchaffersF7FunctionIllConditioned", "") {
  for (const auto& numberOfDimensions : {2, 40}) {
    ...
    expected.load(testDirectory + "/data/optimisationProblem/blackBoxOptimisationBenchmark2015/expectedSchaffersF7FunctionIllConditioned,dim" + std::to_string(numberOfDimensions) +".mat");
  ...
  SECTION("Returns the specified class name.") {
    CHECK(mant::bbob2015::SchaffersF7FunctionIllConditioned(5).toString() == "schaffers-f7-function-ill-conditioned");
  }
}
