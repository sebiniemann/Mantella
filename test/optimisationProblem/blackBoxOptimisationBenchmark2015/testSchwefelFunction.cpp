...
TEST_CASE("bbob2015::SchwefelFunction", "") {
  for (const auto& numberOfDimensions : {2, 40}) {
    ...
    expected.load(testDirectory + "/data/optimisationProblem/blackBoxOptimisationBenchmark2015/expectedSchwefelFunction,dim" + std::to_string(numberOfDimensions) +".mat");
  ...
  SECTION("Returns the specified class name.") {
    CHECK(mant::bbob2015::SchwefelFunction(5).to_string() == "SchwefelFunction");
  }
}
