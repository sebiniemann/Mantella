...
TEST_CASE("bbob2010::DifferentPowersFunction", "") {
  for (const auto& numberOfDimensions : {2, 40}) {
    mant::bbob2010::DifferentPowersFunction differentPowersFunction(numberOfDimensions);
  ...
  SECTION("Returns the specified class name.") {
    CHECK(mant::bbob2010::DifferentPowersFunction(5).toString() == "different-powers-function");
  }
}
