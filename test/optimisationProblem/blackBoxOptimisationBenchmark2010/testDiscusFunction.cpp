...
TEST_CASE("bbob2010::DiscusFunction", "") {
  for (const auto& numberOfDimensions : {2, 40}) {
    mant::bbob2010::DiscusFunction discusFunction(numberOfDimensions);
  ...
  SECTION("Returns the specified class name.") {
    CHECK(mant::bbob2010::DiscusFunction(5).toString() == "discus-function");
  }
}
