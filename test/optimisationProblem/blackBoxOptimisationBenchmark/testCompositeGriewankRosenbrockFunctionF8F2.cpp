...
TEST_CASE(
    "bbob::CompositeGriewankRosenbrockFunctionF8F2") {
  for (const auto& numberOfDimensions : {2, 40}) {
    ...
    REQUIRE(expected.load(testDirectory +
                          "/data/optimisationProblem/blackBoxOptimisationBenchmark/bbob_compositeGriewankRosenbrockFunctionF8F2_dim" + std::to_string(numberOfDimensions) +
                          ".expected"));
  ...
  SECTION(
    ...
    SECTION(
      ...
      CHECK(mant::bbob::CompositeGriewankRosenbrockFunctionF8F2(5).toString() ==
