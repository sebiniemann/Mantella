...
TEST_CASE("bbob::EllipsoidalFunctionRotated") {
  for (const auto& numberOfDimensions : {2, 40}) {
    ...
    REQUIRE(expected.load(testDirectory + "/data/optimisationProblem/blackBoxOptimisationBenchmark/bbob_ellipsoidalFunctionRotated_dim" + std::to_string(numberOfDimensions) +".expected"));
