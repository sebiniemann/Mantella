...
TEST_CASE(
    "quasiRandomSequence: getHaltonSequence(...)") {
  SECTION(
      "Generates a Halton sequence without an offset.") {
  ...
  SECTION(
      "Generates a Halton sequence with an offset.") {
    arma::Mat<double>::fixed<2, 5> expected = {
  ...
  SECTION(
      "Exception tests") {
    SECTION(
        "Throw an exception, if the size of the base and seed mismatch.") {
...
TEST_CASE(
    "quasiRandomSequence: getVanDerCorputSequence(...)") {
  SECTION(
  ...
  SECTION(
      "Generates a Van der Corput sequence with an offset.") {
