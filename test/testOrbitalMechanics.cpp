...
SCENARIO("stumpffFunction", "[orbitalMechanics][stumpffFunction]") {
  GIVEN("A parameter and a type") {
    THEN("Return the result of the Stumpff function") {
      CHECK(mant::stumpffFunction(2.0, 4) == Approx(0.0389859237));
      CHECK(mant::stumpffFunction(-2.0, 5) == Approx(0.0087413847));
    }
  }
}
...
SCENARIO("timeOfFlight", "[orbitalMechanics][timeOfFlight]") {
  GIVEN("A universal variable, a departure position, an arrival position and whether to use a prograde trajectory or not") {
    THEN("Return the time of flight") {
      ...
      CHECK(mant::timeOfFlight(10, {1.3673758920848439e+11, 6.0796174698209099e+10, 6.9558483573525702e+05}, {1.0958474846549156e+10, -1.0758115275186798e+11, -4.1768951438493319e+09}, true) == Approx(8037507.5295110801));

      // From Venus (after one revolution) back towards Earth.
      CHECK(mant::timeOfFlight(80, {1.0958474846549156e+10, -1.0758115275186798e+11, -4.1768951438493319e+09}, {1.2400638592866623e+11, -8.3833980661523087e+10, 3.2922006660828856e+05}, true) == Approx(32369680.0891493782));
    }
  }
}
...
SCENARIO("sphereOfInfluenceRadius", "[orbitalMechanics][sphereOfInfluenceRadius]") {
  GIVEN("A universal variable, a departure position, an arrival position and whether to use a prograde trajectory or not") {
    THEN("Return the time of flight") {
      ...
      CHECK(mant::sphereOfInfluenceRadius(arma::norm(arma::vec({1.0958474846549156e+10, -1.0758115275186798e+11, -4.1768951438493319e+09})), 4.867e24) == Approx(616194206.6671903133));
    }
  }
}
  ...
  GIVEN("A modified Julian date, Keplerian elements and a modified Julian reference date") {
    WHEN("Valid timestamp and keplerian elements are given") {
      THEN("The eccentricity is less than 1.0") {
        ...
        auto positionAndVelocity = mant::positionAndVelocityOnOrbit(4383590400.0, earthKeplerianElements);
        ...
        positionAndVelocity = mant::positionAndVelocityOnOrbit(4400265600.0, venusKeplerianElements);
        ...
        positionAndVelocity = mant::positionAndVelocityOnOrbit(4441651200.0, earthKeplerianElements);
    ...
    WHEN("The eccentricity is equal to or greater than 1.0") {
      THEN("Throw a std::logic_error") {
        CHECK_THROWS_AS(mant::positionAndVelocityOnOrbit(4453401600.0, arma::vec({108209474522.8824, 1.0067767205622176587, 0.059248274299581655, 1.3383157887557473, 0.95858056664193103, 0.87043843382287189, 4453401600.0})), std::logic_error);
      }
    }
  }
}
...
SCENARIO("lambert", "[orbitalMechanics][lambert]") {
  GIVEN("A departure position, an arrival position, whether to use a prograde trajectory or not, a number of revolutions and a transfer time") {
    THEN("Return departure and arrival velocity") {
      // Start of the Cassini-Huygens mission (Earth) towards Venus.
      auto departureAndArivalVelocity = mant::lambert({1.3673758920848439e+11, 6.0796174698209099e+10, 6.9558483573525702e+05}, {1.0958474846549156e+10, -1.0758115275186798e+11, -4.1768951438493319e+09}, false, 0, 16675200.0);
      ...
      // From Venus (after one revolution) back towards Earth.
      departureAndArivalVelocity = mant::lambert({1.0958474846549156e+10, -1.0758115275186798e+11, -4.1768951438493319e+09}, {1.2400638592866623e+11, -8.3833980661523087e+10, 3.2922006660828856e+05}, false, 1, 41385600.0);
