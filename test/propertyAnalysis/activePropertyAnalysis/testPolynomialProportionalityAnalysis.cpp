...
TEST_CASE("QuadraticProportionalityAnalysis") {
  ...
  SECTION(".toString") {
    SECTION("Returns the expected class name.") {
      ...
      CHECK(mant::PolynomialProportionalityAnalysis(optimisationProblem, std::shared_ptr<mant::RegressionFunction>(new mant::LinearRegressionFunction(optimisationProblem->numberOfDimensions_))).toString() == "polynomial_proportionality_analysis");
    }
  }
}
