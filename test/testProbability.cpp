...
TEST_CASE("randomRotationMatrix") {
  SECTION("Generates uniform distributed 2-dimensional rotations.") {
    ...
    for (arma::uword n = 0; n < angles.n_elem; ++n) {
      arma::Col<double>::fixed<2> rotatedUnitVector = mant::randomRotationMatrix(2) * arma::normalise(arma::ones<arma::Col<double>>(2));
  ...
  SECTION("Generates uniform distributed 3-dimensional rotations.") {
    ...
    for (arma::uword n = 0; n < rollAngles.n_elem; ++n) {
      arma::Col<double>::fixed<3> rotatedUnitVector = mant::randomRotationMatrix(3) * arma::normalise(arma::ones<arma::Col<double>>(3));
