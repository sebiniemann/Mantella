...
TEST_CASE("RandomSamplesSelection") {
  ...
  SECTION(".toString") {
    SECTION("Returns the expected class name.") {
      ...
      CHECK(mant::RandomSamplesSelection(optimisationProblem->getCachedSamples(), std::uniform_int_distribution<arma::uword>(1, optimisationProblem->getCachedSamples().size())(mant::Rng::getGenerator())).toString() == "random_samples_selection");
    }
  }
}
