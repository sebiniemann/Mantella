...
TEST_CASE("BestNeighbourhoodSamplesSelection") {
  ...
  SECTION(".toString") {
    SECTION("Returns a (filesystem friendly) name for the class.") {
      ...
      CHECK(mant::BestNeighbourhoodSamplesSelection(optimisationProblem->getCachedSamples(), std::uniform_int_distribution<arma::uword>(1, optimisationProblem->getCachedSamples().size())(mant::Rng::getGenerator())).toString() ==
            "best_neighbourhood_samples_selection");
    }
  }
}
